require 'rubygems'
require 'bundler'
Bundler.require
Dotenv.load

# CHANGE YOUR SESSION KEY
use Rack::Session::Cookie, key: '_rack_demo.session', secret: ''
use OmniAuth::Builder do
  provider :github, ENV['GITHUB_KEY'], ENV['GITHUB_SECRET']
end

class Demo
  def call(env)
    req = Rack::Request.new(env)
    if req.path_info.match '/user'
      [200, {'Content-Type' => 'text/html'}, [File.read('user.html').to_s]]
    elsif req.path_info.match '/auth/github/callback'
      [301, {'Location' => '/user'}, []]
    else
      [200, {'Content-Type' => 'text/html'}, [File.read('signin.html').to_s]]
    end
  end
end

run Demo.new
